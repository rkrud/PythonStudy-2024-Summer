...
def digits_to_words(input_string):
    ...
    num_word = [num[i] for i in input_string]

    digit_string = " ".join(num_word)
...
def to_camel_case(underscore_str):
    ...
    underscore_str_split = underscore_str.split("_")

    if underscore_str == underscore_str_split[0]:
        return underscore_str[0].lower() + underscore_str[1:]
    
    for i in range (len(underscore_str_split)):
        if i == 0:
            underscore_str_split[i] = underscore_str_split[i].lower()
        else:
            if underscore_str_split[i] != "":  
                underscore_str_split[i] = underscore_str_split[i][0].upper() + underscore_str_split[i][1:].lower()
            else:
                underscore_str_split[i] = ""  
                    
    camelcase_str = "".join(underscore_str_split)
